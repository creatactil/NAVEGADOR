function crearLista(arg) {
	
	if ((typeof arg == 'undefined') || (arg == "")){ var arg = false }
	
	var xcorreo = localStorage.correo;
	var validado = localStorage.validado;
	
	if (validado != 1){
		alert("ERROR: Usted no ha sido validado. Contacte con COARCO");
		xcorreo = "";
		
	}
	
	var argumento=arg;	
	$.mobile.changePage("#page4", {transition: "slide", reverse: argumento });
	
		
	comprobarcolor();

	$('#page4 ul').empty();
    $("#carga23").show();
		                 
						
	$.ajax({
		url: 'http://appcoarco.creatactil.com/php/listamensajes.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		data: {numero: xcorreo},
		success: function(data){			
		 
		$.each(data, function(index, item) {
			
			$('#id_'+index).remove();
			
			if(item.estado==0){
				var tema = "#000000";
				var bordeIzq = "border-left: 2px solid #294e85;";
				var colorFondo = "";
				var listo = "#lista";
			}else{	
				var tema = "#808080";
				var bordeIzq = "";
				var colorFondo = "background-color: #ffffff;";
				var listo = "#lista";
			}
			
			if(item.tipo == 'M'){
				
				var tipo = "Información Corporativa";
				var logo = "inf_corporativa.svg";
			}else{
					
				var tipo = "Información Comercial";
				var logo = "inf_comercial.svg";
			}			
							
			if (localStorage.bgcolor) { var bgcolor = localStorage.bgcolor; }else{ var bgcolor = "#e0e0e0"; }

			$(listo).append(
       	'<li class="ui-li-has-alt ui-li-has-thumb" style="border: 2px solid '+bgcolor+';margin-bottom: -2px;"><a href="#" class="ui-btn" onclick="cambiarPaginaCircular(id_' + index + '), contador(\''+item.registro+'\');" style="height: 55px; padding:0; border: none; '+bordeIzq+' '+colorFondo+'" >'+
		'<img src="recursos/'+logo+'" class="ui-li-thumb" style="margin-top: 5px; margin-left: 5px; height: 48px">'+
			
		'<p style="margin-left:65px; margin-top:5px; margin-bottom: 0px; margin-right: 3px; font-weight: bold; color:'+tema+'" >'+item.titulo+'</p>'+
		'<p style="margin-left:65px; margin-top:0px; padding:0; color:#294e85; font-size:0.7em">'+
    	//'Centro:  '+item.nombre_centro+'<br>'+
		''+tipo+'<br>'+
    	'Fecha: '+item.fecha+'</p>'+
		
		'<a href="#" data-rel="popup" data-position-to="window" data-transition="pop" aria-haspopup="true" aria-owns="purchase" aria-expanded="false" class="ui-btn ui-btn-icon-notext ui-icon-delete ui-btn-b" onClick="borrarMensaje(\''+item.id_aux+'\' )" style="border: none; border-left:2px solid '+bgcolor+'; '+colorFondo+'">'+
		'</a>'+
		'</li>'
		);
			
			
		var altura_contenido = $(window).height();
		
		var fileName = item.enlace;
		var fileExtension = fileName.substring(fileName.lastIndexOf('.') + 1);
		
		if (isPdf(fileExtension)) {
			var ver_pdf = "block";
		}else{
			var ver_pdf = "none";
		}
		
		if (isImage(fileExtension)) {
			var enlace_imagen = "http://coarco.creatactil.com/files/"+item.enlace;
			var ver_imagen = "block";
			
		}else{
			var enlace_imagen = "";
			var ver_imagen = "none";
		}

		if(item.firmada == 0){
			var ver_firmar = "block";
			var firmada = "";
		}else{
			var ver_firmar = "none";
			var firmada = "Circular firmada";
		}		
										
		if (localStorage.coloroscuro) { var coloroscuro = localStorage.coloroscuro; }else{ var coloroscuro = "#cacaca"; }			
			
		/*Pagina dinamica*/
		var content = 
		'<div data-role="page" id="id_' + index + '" data-url="id_' + index + '" data-theme="a">' +
		
		'<div data-role="header" data-theme="a">' +
		'<a onClick="crearLista(true)" class="ui-link ui-btn-left ui-btn ui-btn-a ui-icon-carat-l ui-btn-icon-notext ui-shadow ui-corner-all">Atrás</a>'+
		'<h1><img src="recursos/logo.svg" width="20" height="20" style="margin: -4px;margin-right: 5px;">CIRCULAR</h1>' +
		'<a id="favorito" data-role="button" data-icon="star" data-iconpos="notext" data-inline="true" onclick="favorito(\''+item.id_aux+'\');" data-theme="a">Añadir a Favorito</a>'+	
		'</div>' +

		'<div data-role="content" id="contenido" style="padding: 0.5em; min-height: '+altura_contenido+'px;">' +
			
		'<div data-role="fieldcontain" class="result" style="background-color: white; border: 2px solid '+coloroscuro+'; padding: 7px;">'+
		
		'<img src="'+enlace_imagen+'" style="margin-bottom: 1em; max-width: 100%; display: '+ver_imagen+';" onclick="abririmagen(\''+enlace_imagen+'\');">'+

		'<p style="margin:0; padding:0; color:#294e85; font-size: 0.7em ">Nº de Registro: '+item.registro+'<br>'+
		//item.nombre_centro+'<br>'+
		'Fecha: '+item.fecha+'<br>'+
		//'Enviado por: '+item.usuario+'</p>'+
		
		'<p><strong>'+item.titulo+'</strong></p>'+
		'<p>'+ item.texto + '</p>' +
		'<p><strong>'+item.nombre+'</strong></p>'+

		'<img class="boton-pdf" src="recursos/btn-pdf.svg" width="65" height="65" style="margin-bottom: 1.5em; display: '+ver_pdf+';" onclick="abrirfichero(\''+item.enlace+'\')"  />'+
		//'<a href="" onclick="abrirfichero(\''+item.enlace+'\')" >'+item.enlace+'</a>'+

		'<a class="firmaraqui" style="margin-top: 1em; margin-bottom: 1em; color:#294e85; display: '+ver_firmar+';"  data-transition="pop" onClick="irApage11();textofirma(\''+item.registro+'\' );textofirma2(\''+item.titulo+'\' )">'+item.firma+'</a>'+
		
		'<a class="circularfirmada" href="" style="margin-top: 1em; margin-bottom: 1em; color:green;">'+firmada+'</a>'+

		/*'<center>'+
		'<a id="favorito" data-role="button" data-icon="star" data-inline="true" style="margin-top: 1.5em;" onclick="favorito(\''+item.id_aux+'\');" data-theme="a">Añadir a Favoritas</a>'+
		'</center>'+*/
		'</div>' +					  
			
		'</div>';


		var id_circular="#id_"+ index;
		$(document).on("pagecreate",id_circular,function(){
			$(id_circular).off();
			$(id_circular).on("swiperight",function(){
				crearLista(true);
			});
		});		
		
		$('body').append(content).trigger('refresh');
		$(index).page();
			
	});
		$("#carga23").hide(); 
			
	}
		
	});

		$('#lista').listview('refresh', true);
		globo();	
}

function cambiarPaginaCircular(id) {
		$.mobile.changePage($(id), {transition : "slide"});
		comprobarcolor();
}	
	
function borrar1(id_aux){
	
	var elidaux = id_aux; 
	$("#id_aux3").val(elidaux);	
	
}

//Añade al contador de mensaje. Y cambia el estado del mensaje de no leido a leido.
function contador(registro){
	
	var registro3 = registro;
	var xcorreo3 = localStorage.correo;
	
	$.ajax({
		url: 'http://appcoarco.creatactil.com/php/contador.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		data: {numero: xcorreo3, registro: registro3},
		success: function(data){}
		
		});
	//modificaEstado(registro3);
	globo();
}

//Cambia el estado del mensaje de no leido a leido
/* function modificaEstado(registro){
	
	var registro2 = registro;
	var xnumero2 = localStorage.numero;
	
	$.ajax({
		url: 'http://appcoarco.creatactil.com/php/modificaestado.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		data: {numero: xnumero2, registro: registro2},
		success: function(data){}
		
		});
		globo();
} */

function textofirma(registro){
	
	$("#registrofirma").text(registro);

}
	
function textofirma2(titulo){

	$("#titulofirma").text(titulo);
	
}

function abrirfichero(nombre){
	// Para Android y Web
	var server = "http://coarco.creatactil.com/pdfjs1.4.20/web/viewer.html?file=http://coarco.creatactil.com/files/";
	var filename = nombre;
	var uri = encodeURI(server + filename);
	
	// Para IOS
	var server2 = "http://coarco.creatactil.com/files/";
	var uri2 = encodeURI(server2 + filename);
	console.log(uri);
	
	
	if(isPhoneGap() && esIOS()){
	// Para IOS
	window.open (uri2, '_blank', 'location=no,closebuttoncaption=Salir,enableViewportScale=yes');
	
	}else{
	// Para Android y Web
	$(".iframe_pdf").prop('src', uri);
	$.mobile.changePage("#page19", {transition: "slide", reverse: false });	
	//window.open (uri, '_system', 'location=yes,closebuttoncaption=Salir,enableViewportScale=yes');
	
	}
}

function abririmagen(ruta){
	
	// Para iOS utilizamos _blank y para Android y Navegador Web utilizamos _system
	if(isPhoneGap() && esIOS()){		
		window.open (ruta, '_blank', 'location=yes,closebuttoncaption=Salir,enableViewportScale=yes');
	}else{
		window.open (ruta, '_system', 'location=yes,closebuttoncaption=Salir,enableViewportScale=yes');
	}	
	
}


function isPdf(extension){
    switch(extension.toLowerCase()) 
    {
        case 'pdf':
           return true;
       break;
       default:
            return false;
        break;
   }
}

function isImage(extension){
    switch(extension.toLowerCase()) 
    {
        case 'jpg': case 'jpeg': case 'png':
           return true;
       break;
       default:
            return false;
        break;
   }
}